spfclust <-
           ...
           CK=log(20),hard = F, plot= F,trace=F,perc_rankpapp=NA,mu_fd_vero=NA,lambda_s_ini=NA,lambda_s_pen=NA,init="kmeans",breaks=NA,varcon="non-diagonal")
  {
    ...
    initfit <- fclustinit(data = data, pert = pert, grid = grid,  q = q, K = K, der = der, gamma_ada = gamma_ada, mu_fd_vero =
                            mu_fd_vero, lambda_s_ini = lambda_s_ini, lambda_s_pen = lambda_s_pen, init =
                            init, breaks = breaks, varcon = varcon)
    ...
    # the stopping condition is met: abs(lk_old - lk_new) > tol & (ind <= maxit).
    lk_old=0
  ...
  function(data, pert = 0, grid = seq(0.01, 1, length = 100),  q = 5,K = K,der=der,gamma_ada=gamma_ada,mu_fd_vero=mu_fd_vero,lambda_s_ini=lambda_s_ini,lambda_s_pen=lambda_s_pen,init=init,breaks=breaks,varcon=varcon){
    ...
    if(K!=1)
    {
      ...
      for (ii in 1:(K-1)) {
        ...
        if(length(ind[1]:ind[2])==1) aa<--1 else aa<-diag(rep(-1,length(ind[1]:ind[2])))
    ...
    if(!is.na(pert)){#Initialization of James et al. 2003
      ...
      if(length(unique(d))==1&length(unique(e))==1){#For functional data observed over the same grid of points
        basis_start<-basis
      ...
      else{#For functional data observed over different grids of points
        basis_start<-basis
        loglam         = seq(-10, 10, 0.25)
    ...
    # Get initial cluster memberships from points.
    if(K > 1){
    ...
    # Initialize mu, gamma, gprod=gamma%*%t(gamma),Gamma,gcov
    ...
    list(S = S, W = W, AW_vec = AW_vec, P_tot = P_tot, P = P, FullS = FullS, parameters = list(mu = mu_start, sigma = sigma, pi = pi, Gamma = Gamma), vars = list(gamma = gamma, piigivej = piigivej, gprod = gprod, gcov = gcov ), basis = basis)
  }
...
fclustMstep <-
  function(parameters, data, vars, S, tol,  hard,lambda_s,lambda_l,W,AW_vec,P_tot,par_LQA,CK,perc_rankpapp,varcon=varcon)
  {
    ...
    # Get numerator and denominator for the local quadratic approximation step
    ...
    # Local quadratic approximation step
    z_int=1
    diff_inter <- 100
    mu_old=mu
...
fclustEstep <-
  function(parameters, data, vars, S, hard)
  {
    ...
    Estep<-get_Estep(parameters, data, vars, S, hard,n_i)
    vars$gamma=Estep[[1]]
    vars$piigivej=Estep[[2]]
    vars$gprod=Estep[[3]]
    vars$gcov=Estep[[4]]
    vars
  }
...
loglik <- function(parameters, data, vars, FullS,W=NA,AW_vec=NA,P_tot=NA,lambda_s=NA,lambda_l=NA,CK=NA, perc_rankpapp2=NA){
...
classify <- function(mod, data_new=NA){
  ...
  vars<-mod$vars
  if(is.na(data_new)) data=mod$data
  gamma <- vars$gamma
  gcov <- vars$gcov
  curve <- data$curve
  pi <- parameters$pi
...
spfclust_cv<-function(data,num_cluster=6,lambda_l_g=10^seq(-1,2),lambda_s_g=10^seq(-5,-3),gamma_ada=c(1),K_fold=5,test=NA,grid=NA,grid_2=NA,k1=1,k2=0,k3=1,maxit=15,q=30,
  ...
  if(is.na(test)){
    ...
    if(!all(grid==grid_2))stop("Not equal grids between training and test set \n")
    parr_fun<-function(ii){
      parameters<-as.numeric(comb_list[ii,])
      num_clusters_i<-parameters[1]
      lambda_s_i<-parameters[2]
  ...
  if(is.na(test)){
    ...
    try(clusterEvalQ(cl, {library(sasfunclust)}))
    cat("Parallelization")
