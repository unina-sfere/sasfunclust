...
plot_cv<-function(mod,m1=NA,m2=NA,m3=NA,lambda_s_min=NA,...){
  ...
  plot(CV_i,pch=16,cex=0.5,col=2,type="l",xaxt="n",xlab="",ylab="CV",ylim=c(min(CV_i)-max(sd_i),max(CV_i)+max(sd_i)),...)
  ...
  if(is.na(m1)) m1<-mod$ms[1]
  if(is.na(m2)) m2<-mod$ms[2]
  if(is.na(m3)) m3<-mod$ms[3]
  ...
  plot(max_vec_nc,pch=16,cex=0.5,col=2,type="l",xaxt="n",xlab="",ylab="CV fixed G",ylim=c(min(max_vec_nc)-max(sd_vec_nc),max(max_vec_nc)+max(sd_vec_nc)))
  ...
  plot(max_vec_s,pch=16,cex=0.5,col=2,type="l",xaxt="n",xlab="",ylab="CV fixed G and lambda_s",ylim=c(min(max_vec_s)-max(sd_vec_s),max(max_vec_s)+max(sd_vec_s)))
